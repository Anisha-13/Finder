    ...
    methods: {
        fetchData() {
            ...
            fetch(this.api + '/data/').then((res) => {
                ...
                this.hospitalList = data.filter((v) => v.type == "Government")
        ...
        async startSearch(searchq) {
            ...
            this.endpoint1 = await new_res.filter((v) => v.type == "Government")
            // endpoint3 for city
            const res2 = await fetch(this.api + "/data/")
            const new_res2 = await res2.json()
            const new_gov = await new_res2.filter((v) => v.type == "Government")
